...
//shop profile
export const getProfileShopRequest = () => ({
  type: CUSTOMER_GET_SHOP_REQUEST,
});
export const getProfileShopSuccess = (shopDetail) => ({
  type: CUSTOMER_GET_SHOP_SUCCESS,
  payload: shopDetail,
});
export const getProfileShopFailure = (error) => ({
  type: CUSTOMER_GET_SHOP_FAILURE,
  payload: error,
});
...
export const getProfileShop = (shopId) => {
  return (dispatch) => {
    console.log(shopId);
    dispatch(getProfileShopRequest());
    axiosInstance()
        .get(`/shop/${shopId}/`)
        .then((response) => {
          const shopDetail = response.data;
          console.log(shopDetail);
          dispatch(getProfileShopSuccess(shopDetail));
        })
        .catch((error) => {
          // const errors = error.response.data;
          console.log(error);
          const msg =
              error.response.status == 404
                  ? "Shop Does Not Exist!"
                  : "Something Went Wrong!";
          dispatch(getProfileShopFailure(msg));
        });
  };
};
